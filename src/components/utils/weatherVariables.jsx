...
export const ShowWeather = memo((props) => {
  ...
  const WeatherDisplay = memo(() => (
    <div className="grow flex flex-col justify-center max-w-7xl mx-auto w-full px-4 py-3">
      {/* Two-column layout for laptop optimization, centered on page */}
      <div className="flex flex-col lg:flex-row lg:gap-4 justify-center">
        {/* Left column - Main info */}
        <div className="lg:w-3/5 mx-auto max-w-3xl">
          {/* Header section with back button and location */}
          <section className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
            <button 
              className="flex items-center text-blue-400 hover:text-cyan-300 transition-colors duration-300 font-medium cursor-pointer group mb-2 sm:mb-0"
            ...
            <div className="text-center sm:text-right">
              <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-cyan-300">{props.name}, {props.country}</h1>
              
              {props.dayConversion && (
                <div className="mt-1">
                  <p className="text-xl font-medium text-white/90">{props.dayConversion} <span className="text-white/80">
                    ...
                    <span className="text-gray-400 ml-1">(<TimeZoneShow timeZone={props.timeZone} />)</span>
                  </span></p>
                </div>
              )}
            </div>
          ...
          {/* Main weather display card - more compact and centered */}
          <section className="bg-gradient-to-r from-slate-900 via-blue-950 to-slate-900 rounded-xl shadow-[0_8px_30px_rgba(0,0,0,0.3)] p-4 mb-4 border border-blue-900/30 backdrop-blur-sm transition-all duration-300 hover:shadow-[0_10px_40px_rgba(0,0,0,0.4)]">
            <div className="flex items-center justify-center sm:justify-start">
              {/* Weather icon */}
              <div className="relative mr-4">
                <div className="absolute inset-0 bg-blue-600/10 blur-2xl rounded-full"></div>
                <div className="relative">
                  <div className="h-[140px] w-[140px] lg:h-[160px] lg:w-[160px] flex items-center justify-center">
                    <WeatherIcons 
              ...
              <div>
                <p className="font-bold text-2xl mb-2 bg-clip-text text-transparent bg-gradient-to-r from-blue-400 via-cyan-400 to-blue-500">{props.description.toUpperCase()}</p>
                <div className="flex items-center">
                  <FaTemperatureHigh size={24} className="mr-3 text-amber-400" />
                  <span className="text-5xl font-bold text-white">{Math.round(props.temperature)}°C</span>
                </div>
                <p className="mt-1 text-lg text-white/90">Feels like: {Math.round(props.tempFeel)}°C</p>
                <div className="flex mt-1 gap-4">
                  <span className="flex items-center text-blue-300">
                    <IoIosArrowDown className="h-4 w-4 mr-1 text-blue-300" />
                    Min: {Math.round(props.tempMin)}°C
                  </span>
                  <span className="flex items-center text-red-300">
                    <IoIosArrowUp className="h-4 w-4 mr-1 text-red-300" />
          ...
          {/* Detailed weather information - 3 column layout for laptops */}
          <section className="bg-gradient-to-r from-gray-900 via-slate-900 to-gray-900 rounded-xl shadow-[0_8px_30px_rgba(0,0,0,0.3)] p-4 mb-4 border border-blue-900/30 backdrop-blur-sm">
            <h2 className="text-lg font-bold mb-3 pb-1 border-b border-blue-900/50 bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-cyan-300">Weather Details</h2>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              <div className="flex items-center bg-black/30 p-2 rounded-lg backdrop-blur-sm border border-gray-800/50 transition-all duration-300 hover:border-gray-700/50">
                <WiHumidity size={32} className="mr-2 text-blue-400" />
                <div>
                  <p className="font-medium text-blue-300">Humidity</p>
                  <p className="text-base font-bold">{props.humidity}%</p>
                </div>
              </div>
              
              <div className="flex items-center bg-black/30 p-2 rounded-lg backdrop-blur-sm border border-gray-800/50 transition-all duration-300 hover:border-gray-700/50">
                <WiBarometer size={32} className="mr-2 text-purple-400" />
                <div>
                  <p className="font-medium text-purple-300">Pressure</p>
                  <p className="text-base font-bold">{props.pressure} hPa</p>
                </div>
              </div>
              
              <div className="flex items-center bg-black/30 p-2 rounded-lg backdrop-blur-sm border border-gray-800/50 transition-all duration-300 hover:border-gray-700/50">
                <BsCloudFog size={28} className="mr-2 text-gray-400" />
                <div>
                  ...
                  <p className="text-base font-bold">
                    ...
                    <span className="text-xs text-gray-400 ml-1 font-normal">(<VisibilityDesc visibility={props.visibility} />)</span>
                  </p>
                </div>
              </div>
              
              <div className="flex items-center bg-black/30 p-2 rounded-lg backdrop-blur-sm border border-gray-800/50 transition-all duration-300 hover:border-gray-700/50">
                <WiStrongWind size={32} className="mr-2 text-teal-400" />
                <div>
                  <p className="font-medium text-teal-300">Wind</p>
                  <p className="text-base font-bold">{props.windSpeed} m/s <span className="text-xs text-gray-400 font-normal">(<WindForce windSpeed={props.windSpeed} />)</span></p>
                </div>
              </div>
              
              <div className="flex items-center bg-black/30 p-2 rounded-lg backdrop-blur-sm border border-gray-800/50 transition-all duration-300 hover:border-gray-700/50">
                <GiWindsock size={28} className="mr-2 text-cyan-400" />
                <div className="flex-1">
                  <p className="font-medium text-cyan-300">Wind Direction</p>
                  <div className="flex items-center">
                    <div className="h-7 w-7 rounded-full bg-gray-700/70 flex items-center justify-center mr-2 wind-arrow-container">
                      <WindArrow degrees={props.windDegrees} />
                    </div>
                    <p className="text-base font-bold"><WindDirection windDegrees={props.windDegrees} /> @ {props.windDegrees}°</p>
                  </div>
                </div>
              </div>
              ...
              {props.rain !== undefined && (
                <div className="flex items-center bg-black/30 p-2 rounded-lg backdrop-blur-sm border border-gray-800/50 transition-all duration-300 hover:border-gray-700/50">
                  <BsFillCloudRainFill size={28} className="mr-2 text-blue-300" />
                  <div>
                    <p className="font-medium text-blue-200">Rain (last hour)</p>
                    <p className="text-base font-bold">{props.rain} mm</p>
                  </div>
                </div>
              )}
              ...
              {props.precipitation !== undefined && (
                <div className="flex items-center bg-black/30 p-2 rounded-lg backdrop-blur-sm border border-gray-800/50 transition-all duration-300 hover:border-gray-700/50">
                  <BsFillCloudDrizzleFill size={28} className="mr-2 text-blue-200" />
                  <div>
                    <p className="font-medium text-blue-100">Precipitation</p>
                    <p className="text-base font-bold">{props.precipitation}%</p>
                  </div>
                </div>
              )}
              ...
              {props.localSunriseSunsetTimes && (
                <>
                  <div className="flex items-center bg-black/30 p-2 rounded-lg backdrop-blur-sm border border-gray-800/50 transition-all duration-300 hover:border-gray-700/50">
                    <BsFillSunriseFill size={28} className="mr-2 text-yellow-400" />
                    <div>
                      ...
                      <p className="text-base font-bold">
                        ...
                        <span className="text-xs text-gray-400 ml-1 font-normal">(<TimeZoneShow timeZone={props.timeZone} />)</span>
                      </p>
                    </div>
                  </div>
                  
                  <div className="flex items-center bg-black/30 p-2 rounded-lg backdrop-blur-sm border border-gray-800/50 transition-all duration-300 hover:border-gray-700/50">
                    <BsFillSunsetFill size={28} className="mr-2 text-orange-400" />
                    <div>
            ...
            <section className="mb-2">
              <div className="flex justify-center sm:justify-start gap-4">
                <form onSubmit={props.handleSubmit3Hour}>
                  <button 
                    type="submit" 
                    className="px-5 py-2 bg-gradient-to-r from-blue-700 to-blue-600 hover:from-blue-600 hover:to-blue-500 rounded-lg transition-all duration-300 shadow-md hover:shadow-lg hover:-translate-y-0.5 font-medium cursor-pointer"
                  >
                    3 Hour Forecast
                  </button>
                </form>
                ...
                <form onSubmit={props.handleSubmitDaily}>
                  <button 
                    type="submit" 
                    className="px-5 py-2 bg-gradient-to-r from-indigo-700 to-indigo-600 hover:from-indigo-600 hover:to-indigo-500 rounded-lg transition-all duration-300 shadow-md hover:shadow-lg hover:-translate-y-0.5 font-medium cursor-pointer"
                  >
                    Daily Forecast
                  </button>
                </form>
...
export const WeatherPopupContent = memo((props) => {
  ...
  // Determine if we should render in landscape mode
  const isHomePage = props.page !== 'map';
  ...
  // Render landscape layout for home page, portrait for map
  if (isHomePage) {
    return (
      <div className="p-3 w-full">
          {/* Location name with enhanced font */}
          <h3 className="font-bold text-xl mb-3 tracking-tight text-cyan-300">
            {currentLocationWeather.name}
          </h3>
          
          {/* Landscape layout with two columns */}
          <div className="flex flex-row">
              {/* Left column - Weather icon and temperature */}
              <div className="flex flex-col items-center mr-4">
                  ...
                  <p className="capitalize font-medium text-sm text-yellow-300 mt-1">
              ...
              <div className="flex-1 grid grid-cols-2 gap-x-4 gap-y-2">
                  <div className="font-medium">
                    <span className="text-sm block mb-0.5 text-gray-300">
                      Humidity
                    </span>
  ...
  // Original portrait layout for map page
  return (
      <div 
